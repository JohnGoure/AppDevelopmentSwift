...
//print("The value of foo is \(foo)")
// foo is a local variable only accessible in the for loop.
...
// The print statement outside the loop uses the global variable x. The print statement inside the loop uses the local variable x.
...
greeting(greeting: "Howdy", name: "John")
/*:
...
class Car {
    ...
    init(_ make: String, _ model: String, _ year: Int) {
        self.make = make
        self.model = model
        self.year = year
    }
